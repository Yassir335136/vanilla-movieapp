...
<div class="grid gap-6 grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10">
    ...
    <form method="post" action="/favorites/add" autocomplete="off">
        <div class="relative">
            <a href="/movie?id=<?= htmlspecialchars($movie->id) ?>">
                <img class="rounded-lg" src="https://image.tmdb.org/t/p/w500/<?= $movie->poster_path ?>">
            </a>
            <?php if (isset($_SESSION['id'])): ?>
                <input hidden value="<?= htmlspecialchars($movie->id) ?>" name="favorite" />
                <button class="absolute top-0 right-0 bg-red-500 text-white rounded-full text-sm p-2 mt-2 mr-2">
...
<div class="flex justify-center mt-5">
    <?php if ($getPage > 1): ?>
        <a href="/search?query=<?= $search . '&page=' . $getPage - 1; ?>" class="flex items-center justify-center px-3 h-8 mr-3 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
    ...
    <?php if ($count): ?>
        <a href="/search?query=<?= $search . '&page=' . $getPage + 1; ?> " class="flex items-center justify-center px-3 h-8 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
            Next
